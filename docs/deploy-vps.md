...
> 各脚本内容，二者等价）。**01 只负责 Swap + 环境快照**：apt 源/时区/
> 控制台防火墙/SSH 登录策略由阿里云默认 + 运维手动管理，脚本已裁剪。
> **PostgreSQL 走 PGDG 18**（Ubuntu/Debian 布局：配置 `/etc/postgresql/18/main/postgresql.conf`、数据
